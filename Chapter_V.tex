...
\begin{proof} \
	...
	\noindent Define a ring homomorphism
	\[
	\varphi : R[x] \longrightarrow (R/I)[x]
	\]
	by reducing coefficients modulo $I$:
	\[
	\varphi\!\left(\sum a_i x^i\right)
	=
	\sum (a_i + I)x^i.
	\]
	This map is surjective since every polynomial over $R/I$
	has a representative in $R[x]$.
	
	 Its kernel consists of those polynomials whose coefficients
	lie in $I$, that is,
	\[
	\ker \varphi = IR[x].
	\]
...
\begin{problem}{V.4.5}
	Let $R$ be a UFD and let $f \in R[x]$.
	
	\begin{itemize}
		\item $(f) = (\operatorname{cont} f)(\overline f)$, where $\overline f$ is primitive.
...
\begin{proof} \
	...
	\noindent Let $f = \sum a_i x^i$ and define
	\[
	\operatorname{cont} f = \gcd(a_0,\dots,a_n).
	\]
	Write
	\[
	f = (\operatorname{cont} f)\,\overline f,
	\]
	where $\overline f$ has coefficients with no non-unit common divisor,
	so $\overline f$ is primitive. Hence
	\[
	(f) = (\operatorname{cont} f)(\overline f).
	\]
	...
	\noindent Suppose $(f) = (c)(g)$ with $g$ primitive. Then
	\[
	f = c g.
	\]
	Since $g$ is primitive and $R$ is a UFD, Gauss' Lemma implies
	\[
	\operatorname{cont}(f) = u c
	\]
